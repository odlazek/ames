...
const routes = [
    ...
    { path: 'detail', component: _heroes_heroes_component__WEBPACK_IMPORTED_MODULE_4__["HeroesComponent"] }
];
...
let HeroDetailComponent = class HeroDetailComponent {
    ...
    ngOnInit() {
        this.heroService.heroID$
            .subscribe(id => this.getHero(id));
    }
    getHero(heroIDget) {
        const id = heroIDget;
...
let HeroesComponent = class HeroesComponent {
    ...
    add(name, score) {
        ...
        this.heroService.addHero({ name, score })
            .subscribe(hero => {
...
let InMemoryDataService = class InMemoryDataService {
    createDb() {
        const heroes = [
            { id: 1, score: 22, name: 'Thor' },
            { id: 2, score: 1, name: 'Goku' },
            { id: 3, score: 24, name: 'SpiderMan' },
            { id: 4, score: 56, name: 'Joker' },
            { id: 5, score: 53, name: 'Želodko superca' },
        ];
        return { heroes };
    }
...
module.exports = __webpack_require__(/*! C:\Users\Rok Odlazek\Desktop\WEBD\testGit\amesHeros\src\main.ts */"./src/main.ts");
